"""Tests for chat service integration with AgentRouter.

Verifies that create_message routes through the agent router
...
class TestChatAgentRouterIntegration:
    """Test that chat service uses AgentRouter."""
    ...
    async def test_create_message_preserves_response_contract(
        ...
        with patch(
            ...
            assert data["assistant_message"]["content"] == "Resposta via agent router"
    ...
    async def test_completed_onboarding_user_gets_direct_response(
        ...
        with patch(
            ...
            assert response.status_code == 201
            data = response.json()
            assert data["assistant_message"]["content"] == "Resposta normal do router"
